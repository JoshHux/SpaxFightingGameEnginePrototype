...
namespace Spax.Physics2D
{
    ...
    public class AngleJoint : Joint2D
    {
        ...
        internal override void SolveVelocityConstraints(ref SolverData data)
        {
            ...
            data.velocities[indexA].w -= BodyA._invI * FP.Sign(p) * Spax.FPMath.Min(FP.Abs(p), MaxImpulse);
            data.velocities[indexB].w += BodyB._invI * FP.Sign(p) * Spax.FPMath.Min(FP.Abs(p), MaxImpulse);
        }
