    ...
    public class PrismaticJoint : Joint2D
    {
        ...
        internal override void SolveVelocityConstraints(ref SolverData data)
        {
            ...
            if (_enableLimit && _limitState != LimitState.Inactive)
            {
                ...
                if (_limitState == LimitState.AtLower)
                {
                    _impulse.z = Spax.FPMath.Max(_impulse.z, 0.0f);
                }
                else if (_limitState == LimitState.AtUpper)
                {
                    _impulse.z = Spax.FPMath.Min(_impulse.z, 0.0f);
                }
        ...
        internal override bool SolvePositionConstraints(ref SolverData data)
        {
            ...
            if (_enableLimit)
            {
                ...
                if (FP.Abs(_upperTranslation - _lowerTranslation) < 2.0f * Settings.LinearSlop)
                {
                    ...
                    linearError = Spax.FPMath.Max(linearError, FP.Abs(translation));
                    active = true;
                }
                else if (translation <= _lowerTranslation)
                {
                    ...
                    linearError = Spax.FPMath.Max(linearError, _lowerTranslation - translation);
                    active = true;
                }
                else if (translation >= _upperTranslation)
                {
                    ...
                    linearError = Spax.FPMath.Max(linearError, translation - _upperTranslation);
                    active = true;
                }
            }
